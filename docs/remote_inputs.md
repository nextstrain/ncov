...
> Note: these three Auspice JSON files (for both GISAID and GenBank sources) are also uploaded to `s3://nextstrain-data`.
It is these files that you are viewing when you load pages such as [nextstrain.org/ncov/giasaid/global](https://nextstrain.org/ncov/giasaid/global) and [nextstrain.org/ncov/open/globa](https://nextstrain.org/ncov/open/global).
