...
rule diagnose_excluded:
    ...
    shell:
        """
        {python:q} snakemake/scripts/diagnostic.py \
...
checkpoint partition_sequences:
    ...
    shell:
        """
        {python:q} snakemake/scripts/partition-sequences.py \
            --sequences {input.sequences} \
...
rule mask:
    ...
    shell:
        """
        {python:q} snakemake/scripts/mask-alignment.py \
            --alignment {input.alignment} \
            --mask-from-beginning {params.mask_from_beginning} \
            --mask-from-end {params.mask_from_end} \
...
rule proximity_score:
    ...
    shell:
        """
        {python:q} snakemake/scripts/priorities.py --alignment {input.alignment} \
            --metadata {input.metadata} \
            --reference {input.reference} \
...
rule combine_samples:
    ...
    shell:
        """
        {python:q} snakemake/scripts/combine-and-dedup-fastas.py \
            --input {input} \
...
rule adjust_metadata_regions:
    ...
    shell:
        """
        {python:q} snakemake/scripts/adjust_regional_meta.py \
...
rule haplotype_status:
    ...
    shell:
        """
        {python:q} snakemake/scripts/annotate-haplotype-status.py \
...
rule pangolin:
    ...
    shell:
        """
        {python:q} snakemake/scripts/add_pangolin_lineages.py \
            --tree {input.tree} \
...
rule colors:
    ...
    shell:
        """
        {python:q} snakemake/scripts/assign-colors.py \
...
rule recency:
    ...
    shell:
        """
        {python:q} snakemake/scripts/construct-recency-from-submission-date.py \
            --metadata {input.metadata} \
            --output {output} 2>&1 | tee {log}
        """
