...
abstract: "В этом отчете используются общедоступные геномные данные нового коронавируса (нКоВ, nCoV), полученные от GISAID и Genbank, для оценки уровней и моделей распространения вирусной эпидемии. Мы планируем выпускать обновленные отчеты о ситуации по мере того, как новые данные производятся и передаются. Этот сайт оптимизирован для отображения в настольных браузерах."
---
...
* С этого момента начало происходить распространение от человека к человеку, что привело к наблюдаемым случаям.
* Используя оценки общего количества случаев из Имперского колледжа Лондона (Imperial College London) в несколько тысяч случаев, за период с ноября по январь, мы рассчитали репродуктивное число между 1,8 и 3,5, что указывает на быстрый рост.
```
...
Еще слишком рано, чтобы рассчитать смертность, но первые признаки указывают на то, что она значительно ниже чем у ТОРС-КоВ (SARS-CoV).
Число случаев резко возросло отчасти из-за усиления наблюдения и тестирования.
...
Этот анализ стал возможным благодаря быстрому и открытому обмену геномными данными и интерпретацией учеными всего мира (см. Последний слайд с данными по авторству секвенирования).
```
...
Ниже мы видим иллюстрацию с филогенетическим деревом слева, где мутации показаны в виде цветных кружков. Справа - соответствующие последовательности, с мутациями, показанными как цветными кружками.
Мы можем видеть, что последовательности, которые имеют общюю мутацию, объединены.
Когда последовательности связанны вертикальной линией, как например A и B, это означает, что между ними нет различий - они идентичны.
...
Цвета обозначают регион страны или штат США, а ось X отражает дивергенцию нуклеотидов.
...
Вставки, делеции (удаленные участки) и различия на концах генома, скорее всего содержат ошибки, и поэтому мы замаскировали их для целей этого анализа.
...
Две французские последовательности [принадлежат к одной семье](https://www.thelocal.fr/20200129/coronavirus-in-france-what-you-need-to-know) - Китайской паре из Ухани.
