...
**欧洲**  
* 我们继续看到欧洲各地的样本大量混杂在一起，意味着病毒在过去的3-5周中仍在跨境传播。随着疫情控制措施慢慢生效，我们可能会看到病例越来越多地根据所在国家聚类在一起。

**北美洲**   
* 美国境内的传播情况很复杂：在美国的两端采集到的样本的亲缘关系仍然很近。
...
<br><br>
随着最近限制跨境旅行的防疫措施的推行，病毒序列按照地理位置聚类的现象可能会变得更明显。
<!-- There is no right side text -->
...
# [美国的传播情况很复杂，尚待认识](https://nextstrain.org/ncov/2020-03-27?d=tree,map&f_division=Connecticut&label=clade:B1&m=div&p=full)
...
更多的数据可以帮助我们理解，在美国各州高度差异化的应对策略之下，这场深度连接着美国各地的疫情将会如何发展。
<!-- There is no right side text -->
...
<br><br>
根据树上的相对位置，可以看出这两个郡的聚集病例可能来自于不同的输入事件。
<!-- There is no right side text -->
...
但我们有许多有伊朗旅行史的样本序列。
<br><br>
大多数有伊朗旅行史的样本序列聚集到了一起，这暗示了很多伊朗病例是由单次的输入事件所导致的爆发。
<br><br>
在树的根部，我们发现一条新的来自巴基斯坦的序列，该序列来自一位有伊朗旅行史的女性。
值得注意的是，这条序列是其他已知伊朗序列的外类群，但是从时间上来看，这一条与其他的样本是在差不多的时候被取样的。
<br><br>
这个现象可以由多种不同的假说来解释。如果后期获取得更多数据，我们将进一步更新我们的分析。
<!-- There is no right side text -->
...
<br><br>
其中一些在数日之内采集到的样本聚成了一个紧密相关的本地基因簇。
<br><br>
这反映出当地的本地传播至少从3月15日起就开始了（3月15日是属于这个本地传播链中的样本最早被采集到的日期），还可能更早。
<br><br>
INRB（刚果国立生物医学研究所）对早期样本的出色的测序工作，使得我们能够在这里看到在许多其他地方未被察觉到的社区传播。
<!-- There is no right side text -->
...
```auspiceMainDisplayMarkdown
## 总结
#### 接下来的几周内，我们将会观察欧洲疫情，看新的防疫决策是否能够阻碍跨境传播。
<br><br>
#### 我们也会监控南美的疫情，如果获得更多序列，我们就能找出是否已存在更多本地传播的迹象。
<br><br>
#### 美国的疫情应该是一个非常复杂的传播体系，既有国际输入的病例，也发现了在极远的地区之间的传播。在华盛顿州和加利福尼亚州，我们发现了多重本地传播链的证据。
<br><br>
#### 我们发现澳大利亚新南威尔士州存在本地传播。
<br><br>
#### 刚果民主共和国的金莎萨市也存在本地传播。目前我们还没有足够数据判断在非洲其他地区，病毒是否已经在当地传播开来。
```
