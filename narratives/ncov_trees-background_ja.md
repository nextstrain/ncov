...
<br><br>この「変化」パネルでは、水平軸はウイルスゲノムの各領域（すべて3万程度）です。縦軸は、各領域の変動性を示しています。
<br><br>これらの突然変異の1つ、この場合は遺伝子「ORF1b」の コドン314 に従ってツリーに色を付けました。この変異が機能的変異であると考える合理的な理由はありません（つまり、生物学的変化）。遺伝子配列間の関係を定義し、系統樹を構築するために使用するのは、まさにこのような突然変異です。
