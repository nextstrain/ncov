...
if __name__ == '__main__':
    ...
    for n in T.get_terminals():
        if isinstance(dates[n.name], list):
            continue
    ...
    tmrca = np.linspace(
        2019.7, np.min([x['numdate'] for x in tips.values()]) - 0.001, 101)
    ...
    plt.title('TMRCA of 2019-nCov assuming a star tree\nand Poisson '
              'statistics of mutations', fontsize=16)
    plt.xlabel('TMRCA', fontsize=16)
    plt.ylabel('Probability density', fontsize=16)
    ticks = ['2019-10-01', '2019-11-01', '2019-12-01', '2020-01-01']
    plt.legend(loc=2, fontsize=12)
    plt.tick_params(labelsize=12)
    plt.xticks([numeric_date(datetime.datetime.strptime(x, '%Y-%m-%d'))
                for x in ticks], ticks)
