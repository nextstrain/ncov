...
def modify(node):
    """
    Modify the branch to NODE, if NODE is a terminal node and the
    `division_exposure` of NODE differs from the `division`. This creates an
    intermediate branch, with a single child, such that we have
    ...
    try:
        division = node["node_attrs"]["division"]["value"]
    except KeyError:
        division = ""

    try:
        recoded = node["node_attrs"]["division_exposure"]["value"]
    except KeyError:
        recoded = ""
    ...
    # Make a new node which is the INTERMEDIATE (i.e. a single child, which
    # is `node` with slight modifications)
    n = {
...
def collect_division_exposures(values, node):
    try:
        values.add(node["node_attrs"]["division"]["value"])
    except KeyError:
        pass
...
def switch_division(node):
    """
    For a given node, replace "division" with the info in "division_exposure"
    (removing the latter)
    """
    division_exposure = node["node_attrs"].get("division_exposure")
    division = node["node_attrs"].get("division")
    if division and not division_exposure:
        raise Exception(
            "Where there's division we should always have division_exposure")
    node["node_attrs"]["division"] = division_exposure
    # If division and division_exposure are both None, the following will
    # raise a KeyError. Is that intended?
    ...
    with open(sys.argv[3]) as fh:
        for line in fh:
            ...
            if len(fields) == 4 and fields[0] == "division":
                latlongs[fields[1]] = {"latitude": float(fields[2]),
                                       "longitude": float(fields[3])}

    # Modify branches
    modify_branches(input_json["tree"])

    # And now move `division_exposure` to `division` (i.e. what was
    # `division` is gone)
    traverse(input_json["tree"], switch_division)

    # Collect all `division_exposure` values (we may have created ones which
    # didn't previously exist)
    division_values = set()
    traverse(input_json["tree"], partial(collect_division_exposures,
                                         division_values))

    # Ensure the colors & lat-longsare up-to-date
    reset_colors(input_json["meta"]["colorings"], division_values, colors)
    update_latlongs(input_json["meta"]["geo_resolutions"], division_values,
                    latlongs)

    # Remove `division_exposure` from filters - we only included it so that
    # `augur_export` would include the trait on the nodes.
    input_json["meta"]["filters"] = [f for f in input_json["meta"]["filters"]
                                     if f != "division_exposure"]
