...
class TraverseImpl(Traverse):
    def traverse(self, node, fun):
        """ Traverse the tree calling `fun(node)` on each node """
        fun(node)
        for child in node.get("children", []):
            self.traverse(self, child, fun)
...
class ModifyImpl(Modify):
    def modify_branches(self, node):
        """ Traverse the tree calling `fun` on each child """
        for idx, child in enumerate(node.get("children", [])):
            node["children"][idx] = self.modify(child)
            self.modify_branches(child)
...
class Collect_Exposures(ABC):
    @abstractmethod
    def collect_exposures(self, values, node):
        pass


class Collect_Exposures_IMPL(Collect_Exposures):
...
class SAMP_TRAIT(ABC):
    @abstractmethod
    def switch_attribute(self, node):
        pass

    def reset_colors(self, colorings, values_wanted, colors):
        pass


class SAMP_TRAIT_IMPL(SAMP_TRAIT):
    ...
    def switch_attribute(self, node):
        ...
        if exposure:
            ...
            if "exposure_backup" in node["node_attrs"] and node["node_attrs"]["exposure_backup"]["value"] != \
                    node["node_attrs"][EXPOSURE_TRAIT]["value"]:
    ...
    def reset_colors(self, colorings, values_wanted, colors):
        ...
        for [name, value] in colors:
            if name.lower() in values_wanted_lower:
                ...
                print(
                    "Heads up: a colour has been set for SAMPLING_TRAIT -> {} but it is not found in the data!".format(
                        name))
        for missing_name in values_wanted_lower - values_added:
            print("WARNING: Colour for {} -> {} is missing & auspice will choose a shade of grey for this.".format(
                SAMPLING_TRAIT, missing_name))


class Update_latlongs(ABC):
    @abstractmethod
    def update_latlongs(self, geo_resolutions, values_wanted, latlongs):
        pass


class Update_latlongs_IMPL(Update_latlongs):
    def update_latlongs(self, geo_resolutions, values_wanted, latlongs):
        ...
        for x in values_wanted:
            if x not in trait["demes"].keys():
                try:
                    ...
                    print(
                        "WARNING: The lat/long value for {} -> {} is missing from the lat/longs TSV. Please add it!".format(
                            SAMPLING_TRAIT, x))
...
if __name__ == '__main__':
    ...
    """Object Creation"""
    traverse = Factory.getTraverse()
    modify = Factory.getmodify()
    exposure = Factory.getcollectexposure()
    samptrait = Factory.getsamptrait()
    updatelatlong = Factory.getupdatedlatlong()
    ...
    # Modify branches
    modify.modify_branches(input_json["tree"])

    # And now move EXPOSURE_TRAIT to `sampling` (i.e. what was `sampling` is gone)
    traverse.traverse(input_json["tree"], samptrait.switch_attribute)

    # Collect all EXPOSURE_TRAIT values (we may have created ones which didn't previously exist)
    sampling_values = set()
    traverse.traverse(input_json["tree"], partial(exposure.collect_exposures, sampling_values))

    # Ensure the colors & lat-longsare up-to-date
    samptrait.reset_colors(input_json["meta"]["colorings"], sampling_values, colors)
    updatelatlong.update_latlongs(input_json["meta"]["geo_resolutions"], sampling_values, latlongs)
