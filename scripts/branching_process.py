...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=("Estimate Tmrca assuming a star topology and a Poisson "
                     "mutation process"),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument("--infectious-period", type=float, default=10.0,
                        help="infections period in days")
    parser.add_argument("--start", default='2019-11-27',
                        help="start of the outbreak")
    parser.add_argument("--population", nargs='+', type=int,
                        default=[300, 1000, 3000, 10000],
                        help="number of individuals now")
    parser.add_argument("--output", required=True,
                        help="figure file for line graph")
    args = parser.parse_args()

    d = 365.0 / args.infectious_period
    n_vals = np.array(args.population)

    T = numeric_date() - numeric_date(
    ...
    fs = 16
    plt.figure()
    plt.title(f"Start: {weeks} weeks ago = {args.start}. "
              f"Infectious period {inf_period} days")
    for n in n_vals:
        lh = LH(n, T, b_vals, d)
        lh /= lh.sum()
