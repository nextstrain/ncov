...
def check_additional_info(additional_info, path_to_config_files):
    ...
    for key in sorted_info:
        ...
        while True:
            ...
            if info.lower().startswith("zip") or info.lower().startswith("other: zip") or info.lower().startswith("postal code") or info.endswith(".0") or info.lower() in auto_comments or info.split("Other: ").pop().isnumeric() or (info.isnumeric() and int(info) <= 9999 and int(info) >= 1000):
