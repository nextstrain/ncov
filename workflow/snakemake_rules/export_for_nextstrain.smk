...
# This cleans out files to allow re-run of 'normal' run (not GISAID)
# with `export` to check lat-longs & orderings
# However, *removes* the GISAID files so that when doing final run after all
...
# Just runs this, not GISAID, to speed up & reduce errors.
